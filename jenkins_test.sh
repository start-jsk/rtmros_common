...
function test-grxui {
    ...
    (cd `rospack find $package`; rosrun rosdoc rosdoc $package 2>&1 | tee build/rosdoc.log; [ `grep WARNING build/rosdoc.log | wc -l` -gt 2 ] && exit 1 || true) # we assume .static does not exist... warning
