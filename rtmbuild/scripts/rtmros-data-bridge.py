...
############################################################
# Usage
# . rosrun rtmbuild rtmros-data-bridge.py _input_topic:='TOPIC1 ..' _output_topic:='TOPIC2 ..'
# . cd /tmp
# . rtprint -m RTMROSDataBridge /..../RTMROSDataBridge0.rtc:<topicname>
...
class RtmRosDataIdl:
    ...
    def add_msg_definition(self, msg):
        ...
        # generate idl module
        base = '#include "BasicDataType.idl"\nmodule RTMROSDataBridge\n{%s};'
        open(self.idldir+'/'+self.idlfile, 'w').write(base % self.idltext)

        genidl_bin = Popen(['rtm-config','--idlc'], stdout=PIPE, stderr=PIPE).communicate()[0]
        rtmpkg = Popen(['rtm-config','--prefix'], stdout=PIPE, stderr=PIPE).communicate()[0]
