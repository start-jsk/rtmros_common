...
macro(_rtmbuild_genbridge_init)
  ...
  foreach(_idl_file ${${PROJECT_NAME}_idl_files})
    ...
    execute_process(COMMAND ${idl2srv_EXECUTABLE} -i ${_idl_file} --include-dirs="${rtm_idldir} ${hrp_idldir}" --package-name=${PROJECT_NAME} --tmpdir=/tmp/idl2srv_${PROJECT_NAME}_${_idl_name}_${_rand_str} OUTPUT_VARIABLE ${PROJECT_NAME}_autogen_files OUTPUT_STRIP_TRAILING_WHITESPACE RESULT_VARIABLE _idl2srv_failed ERROR_VARIABLE _idl2srv_error)
    if(DEBUG_RTMBUILD_CMAKE)
    ...
    if ( _idl2srv_failed )
      ...
      message(WARNING ">> ${idl2srv_EXECUTABLE} -i ${_idl_file} --include-dirs=\"${rtm_idldir} ${hrp_idldir}\" --package-name=${PROJECT_NAME} --tmpdir=/tmp/idl2srv_${PROJECT_NAME}_${_idl_name}_${_rand_str}")
    ...
    if ( ${PROJECT_NAME}_autogen_files )
      ...
      add_custom_command(OUTPUT ${${PROJECT_NAME}_${_idl_name}_autogen_cpp_files}
        COMMAND ${idl2srv_EXECUTABLE} -i ${_idl_file} --include-dirs="${rtm_idldir} ${hrp_idldir}" --package-name=${PROJECT_NAME} --tmpdir=/tmp/idl2srv_${PROJECT_NAME}_${_idl_name}_${_rand_str}
        DEPENDS ${_idl_file})
