...
#      OUTPUT_STRIP_TRAILING_WHITESPACE)
  ...
  endforeach(_idl)
  ##
  add_dependencies(rtmbuild_${_project}_genidl RTMBUILD_${_project}_genidl)
  add_custom_target(RTMBUILD_${_project}_genidl ALL DEPENDS ${_autogen})
  if (${CATKIN_TOPLEVEL})
...
macro(rtmbuild_init)
  ...
  add_custom_target(rtmbuild_${_project}_genidl ALL)
  add_custom_target(rtmbuild_${_project}_genbridge ALL)
  ...
  if (${CATKIN_TOPLEVEL})
    ...
    execute_process(COMMAND rospack depends-manifests openrtm_ros_bridge OUTPUT_VARIABLE ${_prefix}_DIRS
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    string(REPLACE " " ";" ${_prefix}_DIRS ${${_prefix}_DIRS})
  else()
...
macro(rtmbuild_add_executable exe)
  if (${CATKIN_TOPLEVEL})
    add_executable(${ARGV})
    add_dependencies(${exe} RTMBUILD_${_project}_genidl ${${_package}_EXPORTED_TARGETS} ${catkin_EXPORTED_TARGETS})
    ...
    add_dependencies(${exe} RTMBUILD_${_project}_genidl)
    target_link_libraries(${exe} ${${_prefix}_IDLLIBRARY_DIRS}${catkin_LIBRARIES})
  endif()
#  rosbuild_add_executable(${ARGV})
...
macro(rtmbuild_add_library lib)
  if (${CATKIN_TOPLEVEL})
    add_library(${ARGV})
    add_dependencies(${lib} RTMBUILD_${_project}_genidl ${${_package}_EXPORTED_TARGETS} ${catkin_EXPORTED_TARGETS})
