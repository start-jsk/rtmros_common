...
macro(rtmbuild_genidl)
  message("[rtmbuild_genidl] Compiling idls in package _${PROJECT_NAME}")
  ...
  if(NOT _idllist)
    ...
    file(MAKE_DIRECTORY ${PROJECT_SOURCE_DIR}/idl_gen/cpp/_${PROJECT_NAME}/idl)
    file(MAKE_DIRECTORY ${PROJECT_SOURCE_DIR}/idl_gen/lib)
  endif(NOT _idllist)

  set(_output_dir ${PROJECT_SOURCE_DIR}/idl_gen)
  set(_output_cpp_dir ${PROJECT_SOURCE_DIR}/idl_gen/cpp/_${PROJECT_NAME}/idl)
  set(_output_python_dir ${PROJECT_SOURCE_DIR}/src/_${PROJECT_NAME})
...
#      OUTPUT_STRIP_TRAILING_WHITESPACE)
    ...
    set(_output_idl_py ${PROJECT_SOURCE_DIR}/src/_${PROJECT_NAME}/${_idl})
    ...
    add_custom_command(OUTPUT ${_output_stub_lib}
      COMMAND ${_rtmcxx_exe} `${_openrtm_aist_pkg_dir}/bin/rtm-config --cflags` -I. ${_${PROJECT_NAME}_IDLLIBRARY_INCDIRS} -shared -o ${_output_stub_lib} ${_output_stub_cpp} `${_openrtm_aist_pkg_dir}/bin/rtm-config --libs` ${OPENHRP_PRIVATE_LIBRARIES}
      DEPENDS ${_output_stub_cpp} ${_output_stub_h})
    add_custom_command(OUTPUT ${_output_skel_lib}
      COMMAND ${_rtmcxx_exe} `${_openrtm_aist_pkg_dir}/bin/rtm-config --cflags` -I. ${_${PROJECT_NAME}_IDLLIBRARY_INCDIRS} -shared -o ${_output_skel_lib} ${_output_skel_cpp} `${_openrtm_aist_pkg_dir}/bin/rtm-config --libs` ${OPENHRP_PRIVATE_LIBRARIES}
      DEPENDS ${_output_skel_cpp} ${_output_skel_h})
    # python
    add_custom_command(OUTPUT ${_output_idl_py}
      COMMAND mkdir -p ${_output_python_dir}
      COMMAND echo \"import sys\; sys.path.append('${PROJECT_SOURCE_DIR}/src/_${PROJECT_NAME}')\; import _${PROJECT_NAME}\" > ${_output_python_dir}/__init__.py
...
macro(rtmbuild_init)
  ...
  message("[rtmbuild] Building package _${PROJECT_NAME}")
  ...
  list(APPEND _${PROJECT_NAME}_INCLUDE_DIRS ${PROJECT_SOURCE_DIR}/idl_gen/cpp)
  list(APPEND _${PROJECT_NAME}_LIBRARY_DIRS ${PROJECT_SOURCE_DIR}/idl_gen/lib)
  list(APPEND _${PROJECT_NAME}_IDL_DIRS ${PROJECT_SOURCE_DIR}/idl)

  if (${CATKIN_TOPLEVEL})
    message(";; rospack will run....")
    execute_process(COMMAND rospack depends-manifests openrtm_ros_bridge OUTPUT_VARIABLE _${PROJECT_NAME}_DIRS
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    string(REPLACE " " ";" _${PROJECT_NAME}_DIRS ${_${PROJECT_NAME}_DIRS})
  else()
    rosbuild_invoke_rospack(${PROJECT_NAME} _${PROJECT_NAME} DIRS depends-manifests)
  endif()
  foreach(_dir ${_${PROJECT_NAME}_DIRS})
    if("${_dir}" MATCHES ".*manifest.xml")
      string(REPLACE "manifest.xml" "idl_gen/cpp" _cpp_dir ${_dir})
      list(APPEND _${PROJECT_NAME}_INCLUDE_DIRS ${_cpp_dir})
      string(REGEX REPLACE ".*/([^/]*)/manifest.xml" "\\1" _tmp_pkg ${_dir})
      list(APPEND _${PROJECT_NAME}_INCLUDE_DIRS ${_cpp_dir}/${_tmp_pkg}/idl)
      string(REPLACE "manifest.xml" "idl_gen/lib" _lib_dir ${_dir})
      list(APPEND _${PROJECT_NAME}_LIBRARY_DIRS ${_lib_dir})
      string(REPLACE "manifest.xml" "idl" _idl_dir ${_dir})
      list(APPEND _${PROJECT_NAME}_IDL_DIRS ${_idl_dir})
    else()
  ...
  list(APPEND _${PROJECT_NAME}_INCLUDE_DIRS ${OPENRTM_INCLUDE_DIRS})
  list(APPEND _${PROJECT_NAME}_INCLUDE_DIRS ${OPENHRP_INCLUDE_DIRS})
  list(APPEND _${PROJECT_NAME}_LIBRARY_DIRS ${OPENRTM_LIBRARY_DIRS})
  list(APPEND _${PROJECT_NAME}_LIBRARY_DIRS ${OPENHRP_LIBRARY_DIRS})
  include_directories(${_${PROJECT_NAME}_INCLUDE_DIRS})
  link_directories(${_${PROJECT_NAME}_LIBRARY_DIRS})
  foreach(_lib ${OPENRTM_LIBRARIES})
    list(APPEND _${PROJECT_NAME}_LIBRARIES ${_lib})
  endforeach(_lib)
  ...
  # get all libraries
  foreach(_idl_dir ${_${PROJECT_NAME}_IDL_DIRS})
    ...
    foreach(_idl_file ${_idl_files})
      ...
      list(APPEND _${PROJECT_NAME}_IDLLIBRARY_DIRS ${_idl_skel_file})
      list(APPEND _${PROJECT_NAME}_IDLLIBRARY_DIRS ${_idl_stub_file})
    endforeach(_idl_file)
    string(REGEX REPLACE ".*/([^/]*)/idl" "\\1" _tmp_pkg ${_idl_dir})
    list(APPEND _${PROJECT_NAME}_IDLLIBRARY_IDLDIRS "-I${_idl_dir}")
    list(APPEND _${PROJECT_NAME}_IDLLIBRARY_INCDIRS "-I${_idl_dir}/../idl_gen/cpp/${_tmp_pkg}/idl")
  endforeach(_idl_dir)
...
macro(rtmbuild_add_executable exe)
  if (${CATKIN_TOPLEVEL})
    ...
    target_link_libraries(${exe} ${catkin_LIBRARIES} ${_${PROJECT_NAME}_IDLLIBRARY_DIRS} )
    ...
    target_link_libraries(${exe} ${_${PROJECT_NAME}_IDLLIBRARY_DIRS}${catkin_LIBRARIES})
  endif()
#  rosbuild_add_executable(${ARGV})
...
macro(rtmbuild_add_library lib)
  if (${CATKIN_TOPLEVEL})
    ...
    target_link_libraries(${lib} ${_${PROJECT_NAME}_IDLLIBRARY_DIRS})
